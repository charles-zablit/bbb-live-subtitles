...
def start_kaldi(server, input, output, controlChannel, speaker, language):
    if language == 'German':
        ...
        kaldiDir = '. path.sh && python nnet3_model.py -m 0 -e -t -y models/%s --redis-server=%s --redis-audio=%s --redis-channel=%s --redis-control=%s -s="%s" -fpc 190' % (model, server, input, output, controlChannel, speaker)
    else:
